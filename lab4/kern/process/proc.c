...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        // LAB4:EXERCISE1 YOUR CODE 2312668 李佳
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 3. 调用 copy_mm 复制/共享内存管理信息（内核线程无需复制，current->mm 为 NULL）
    if (copy_mm(clone_flags, proc) != 0) {
        goto bad_fork_cleanup_kstack; 
    }

    // 4. 调用 copy_thread 复制父进程上下文
